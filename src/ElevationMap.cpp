 ...
 * @brief 高程网格核心：点融合、多假设高度、融合上下界、可见性清理与发布。
 *
 * 与 ROS1 版算法一致；ROS2 差异主要在时间类型 `rclcpp::Time` 与订阅/发布 API。
 */
...
namespace {
/** 将 32 位无符号整数的位模式 reinterpret 为 float（用于 time 等图层编码）。 */
...
ElevationMap::ElevationMap(rclcpp::Node::SharedPtr node)
  ...
  elevationMapFusedPublisher_ =
      node_->create_publisher<grid_map_msgs::msg::GridMap>("elevation_map", rclcpp::QoS(1).transient_local());
  if (!parameters.underlyingMapTopic_.empty()) {
    underlyingMapSubscriber_ = node_->create_subscription<grid_map_msgs::msg::GridMap>(
        parameters.underlyingMapTopic_, rclcpp::QoS(1),
        std::bind(&ElevationMap::underlyingMapCallback, this, std::placeholders::_1));
  }
  visibilityCleanupMapPublisher_ =
      node_->create_publisher<grid_map_msgs::msg::GridMap>("visibility_cleanup_map", rclcpp::QoS(1).transient_local());
...
bool ElevationMap::add(const PointCloudType::Ptr pointCloud, Eigen::VectorXf& pointCloudVariances, const rclcpp::Time& timestamp,
  ...
  const auto duration = std::chrono::duration<double>(std::chrono::steady_clock::now() - methodStartTime).count();
  RCLCPP_DEBUG(node_->get_logger(), "Raw map has been updated with a new point cloud in %f s.", duration);
  return true;
}
...
bool ElevationMap::fuse(const grid_map::Index& topLeftIndex, const grid_map::Index& size) {
  ...
  const auto duration = std::chrono::duration<double>(std::chrono::steady_clock::now() - methodStartTime).count();
  RCLCPP_DEBUG(node_->get_logger(), "Elevation map has been fused in %f s.", duration);
...
void ElevationMap::visibilityCleanup(const rclcpp::Time& updatedTime) {
  ...
  const auto duration = std::chrono::duration<double>(std::chrono::steady_clock::now() - methodStartTime).count();
  RCLCPP_DEBUG(node_->get_logger(), "Visibility cleanup has been performed in %f s (%zu points).", duration,
                cellPositionsToRemove.size());
  if (duration > parameters.visibilityCleanupDuration_) {
    RCLCPP_WARN(node_->get_logger(), "Visibility cleanup duration is too high (current rate is %f).", 1.0 / duration);
  }
}
...
rclcpp::Time ElevationMap::getTimeOfLastFusion() {
  boost::recursive_mutex::scoped_lock scopedLock(fusedMapMutex_);
  return rclcpp::Time(static_cast<int64_t>(fusedMap_.getTimestamp()), node_->get_clock()->get_clock_type());
}
...
void ElevationMap::underlyingMapCallback(const grid_map_msgs::msg::GridMap::SharedPtr underlyingMapMsg) {
  const Parameters parameters{parameters_.getData()};
  RCLCPP_INFO(node_->get_logger(), "Updating underlying map.");
  grid_map::GridMapRosConverter::fromMessage(*underlyingMapMsg, underlyingMap_);
