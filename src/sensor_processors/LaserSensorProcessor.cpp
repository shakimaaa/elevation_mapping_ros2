/**
 * @file LaserSensorProcessor.cpp
 * @author Beauhowe Zhang <zbohao7@gmail.com>
 * @brief 激光测距方差：结合测距误差与扫描角误差传播到高度不确定性。
 */
...
LaserSensorProcessor::LaserSensorProcessor(rclcpp::Node::SharedPtr node, std::shared_ptr<tf2_ros::Buffer> tf_buffer,
...
bool LaserSensorProcessor::readParameters() {
  ...
  parameters.sensorParameters_["min_radius"] = getSensorDouble("min_radius", 0.0);
  parameters.sensorParameters_["beam_angle"] = getSensorDouble("beam_angle", 0.0);
  parameters.sensorParameters_["beam_constant"] = getSensorDouble("beam_constant", 0.0);
  return true;
}
