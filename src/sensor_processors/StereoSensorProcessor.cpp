/**
 * @file StereoSensorProcessor.cpp
 * @author Beauhowe Zhang <zbohao7@gmail.com>
 * @brief 双目点云有序网格索引与视差相关方差计算实现。
 */
...
StereoSensorProcessor::StereoSensorProcessor(rclcpp::Node::SharedPtr node, std::shared_ptr<tf2_ros::Buffer> tf_buffer,
                                             const SensorProcessorBase::GeneralParameters& generalParameters)
    : SensorProcessorBase(std::move(node), std::move(tf_buffer), generalParameters), originalWidth_(1) {}
...
bool StereoSensorProcessor::readParameters() {
  ...
  parameters.sensorParameters_["p_1"] = getSensorDouble("p_1", 0.0);
  parameters.sensorParameters_["p_2"] = getSensorDouble("p_2", 0.0);
  parameters.sensorParameters_["p_3"] = getSensorDouble("p_3", 0.0);
  parameters.sensorParameters_["p_4"] = getSensorDouble("p_4", 0.0);
  parameters.sensorParameters_["p_5"] = getSensorDouble("p_5", 0.0);
  parameters.sensorParameters_["lateral_factor"] = getSensorDouble("lateral_factor", 0.0);
  parameters.sensorParameters_["depth_to_disparity_factor"] = getSensorDouble("depth_to_disparity_factor", 0.0);
